#Loading Required Libraries and Data Setup
library(nlme)
library(sjstats)
library(lme4)
library(lme4)
...
#Creating Histograms of Growth Traits
hist(df$Asymptote)
hist(df$Asymptote1)
...
#mIXED EFECT MODEL for Asymptote
...
#Calculating Model Metrics for Asymptote
rmse(Asym)
summary(Asym)
cv(Asym)
MuMIn::r.squaredGLMM(Asym)

#Extracting Variance Components for Asymptote
...
#Extracting and Plotting BLUPs for Asymptote
Blup_asym <- coef(Asym)$'Pedigree'
hist(Blup_asym$`(Intercept)`)

##Repeated Analysis for Other Traits
##############################################
...
write.csv(fp_subset, "Tpht_subset.csv")
...
head(df)
write.csv(df, "Var_comps_manual_phenos_Tpht.csv")
...
write.csv(dff, "blups_manual_phenos.csv")
