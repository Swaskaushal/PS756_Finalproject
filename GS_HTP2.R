...
setwd("D:/OldPC/D Data Drive/BREEDING/2024/Quantitativegenetics/Finalproject/Data")

#Loading and Preparing Data
df <- read.csv("Phenomic_scaled_centered.csv", row.names = 1)

#Subsetting Data Based on Columns and Computing Cross-Product Matrices
df[1:5,1:5]
df_sub <- df
...
#Loading Additive (AM) and Dominance (DM) Matrices
...
#Setting Up Kernels (Models) for Prediction
# Kernels #
...
foreach(MODEL = 1:length(Models), .packages = c("BGLR", "dplyr")) %dopar% {
  for (rep_num in 1:25)
  {
    ...
    for(fold_num in 1:max(folds))
    {
      ...
      fit<-BGLR(y=y_t,ETA=Models[[MODEL]],nIter=100,burnIn=100, thin=10)
      yield$yhat <- fit$yHat
      # CV1_M1 #
